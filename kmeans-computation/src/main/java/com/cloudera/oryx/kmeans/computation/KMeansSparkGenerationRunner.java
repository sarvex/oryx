...
import com.cloudera.oryx.kmeans.computation.modelbuilder.KMeansModelBuilder;
...
public class KMeansSparkGenerationRunner extends LocalGenerationRunner {

  public static final String CONFIG_SERIALIZATION_KEY = "CONFIG_SERIALIZATION";

  private static final Logger log = LoggerFactory.getLogger(KMeansSparkGenerationRunner.class);

  private final String sparkMaster;

  public KMeansSparkGenerationRunner() { this(ConfigUtils.getDefaultConfig().getString("model.spark-master")); }

  public KMeansSparkGenerationRunner(String sparkMaster) {
  ...
  protected void runSteps() throws IOException, JobException, InterruptedException {
    ...
    KMeansModelBuilder modelBuilder = new KMeansModelBuilder();
    KMeansJobStepConfig jobStepConfig = new KMeansJobStepConfig(
        getInstanceDir(),
        getGenerationID(),
        getLastGenerationID(),
        0);
    ...
    JavaSparkContext jsc = new JavaSparkContext(sparkMaster, "KMeans", sparkConf);
    ...
    SparkPipeline sp = new SparkPipeline(jsc, "KMeans");
    ...
    modelBuilder.build(toRecords(sp.readTextFile(generationPrefix + "inbound/")), jobStepConfig);

    sp.done();
    jsc.stop();
  }

  static PCollection<Record> toRecords(PCollection<String> lines) {
